...
export class SecureChannel {
    ...
    async encrypt(key, seq, plaintext) {
        const nonce = this.nonce(seq);
        const subtle = subtleCrypto();
        if (!subtle)
            return gcm(key, nonce, this.aad).encrypt(plaintext);
        const imported = await subtle.importKey('raw', key, 'AES-GCM', false, ['encrypt']);
        const sealed = await subtle.encrypt({ name: 'AES-GCM', iv: nonce, additionalData: this.aad }, imported, plaintext);
        return new Uint8Array(sealed);
    }
    async decrypt(key, seq, ciphertext) {
        const nonce = this.nonce(seq);
        const subtle = subtleCrypto();
        if (!subtle)
            return gcm(key, nonce, this.aad).decrypt(ciphertext);
        const imported = await subtle.importKey('raw', key, 'AES-GCM', false, ['decrypt']);
        const opened = await subtle.decrypt({ name: 'AES-GCM', iv: nonce, additionalData: this.aad }, imported, ciphertext);
        return new Uint8Array(opened);
    }
    /** Seals a request as `seq || AES-GCM(len || header || body)`. */
    async sealRequest(seq, request) {
    ...
    async openResponse(seq, envelope) {
        const plaintext = await this.decrypt(this.serverToClient, seq, envelope);
        if (plaintext.length < 4)
            throw new Error('Envelope response is truncated');
        const headerLength = new DataView(plaintext.buffer, plaintext.byteOffset).getUint32(0, false);
        if (plaintext.length < 4 + headerLength)
            throw new Error('Envelope header length is out of range');
        const header = JSON.parse(decoder.decode(plaintext.subarray(4, 4 + headerLength)));
        return {
    ...
    async openRequest(envelope) {
        if (envelope.length < 8)
            throw new Error('Envelope is too short to contain a sequence number');
    ...
    async sealResponse(seq, response) {
 ...
 */
export function buildMultipartBody(fieldName, fileName, mimeType, bytes) {
